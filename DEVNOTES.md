...
**The first compromise is practical: the 3:1 compute ratio.** A model has a forward pass and a backward pass. The forward is essentially `x · W` compute. The backward has to produce two things: the parameter gradient `dL/dW` (used to update the weights) and the data gradient `dL/dx` (the chain-rule intermediate that propagates error to the previous layer). Each of those costs about as much as the forward, so the backward is ≈ 2× the forward, and training is therefore ≈ **3× the compute of inference**: one forward plus two backward passes (one for the data gradient, one for the parameter gradient). (A tempting "corner" that doesn't actually exist for an LLM: the first layer looks like it could skip its data gradient `dL/dx`, since its input comes from a discrete token lookup. But that `dL/dx` *is* the embedding table's gradient — the lookup's backward is a scatter of exactly that tensor into the embedding rows — so as long as the embeddings train (here they do, tied to the LM head), nothing is skipped; only a frozen-embedding model gets the hair-under-3:1 discount. The last layer has no shortcut either: the LM head needs both its weight gradient *and* to seed the backward chain, `dL/dhidden`.)

**So why not reuse one forward for both training and rollout, and make it 3:0?** The question hides a direction-of-time error and an arithmetic error, and they're worth separating. Reuse in one direction is simply impossible: you cannot compute the training forward first and replay it as the rollout, because the training forward is a **prefill** — every position of an *already-known* sequence processed at once as one large matmul — and at generation time that sequence doesn't exist yet; producing it is the rollout's whole job. Token *t+1* depends on the model's output at token *t*, so generation is inherently autoregressive: you **decode** one token at a time. The two regimes also perform very differently, which will matter in a moment: prefill is **compute-bound** — each weight is reused across all the tokens in the batch, so the tensor cores stay saturated (high MFU) — while decode is **memory-bandwidth-bound**: to emit a single token you must stream every weight (and the whole KV cache) out of HBM with almost no arithmetic reuse, so the tensor cores starve and MFU collapses (batching many rollouts amortizes the weight reads, but the serial dependency remains; this is also why the KV cache, discussed below, matters so much).

Reuse in the *other* direction is causally fine — the rollout runs first, so its forward could in principle be stored and stand in for the training forward. But mind the arithmetic: that eliminates the *training* forward, not the rollout — the step goes from 1 + 3 to 1 + 2, and the backward's 2 is untouchable. **The best reuse can ever offer is 2:1, not 3:0** — the rollout forward can never be deleted, because it is the thing that generates the data. (The one setting where the two forwards genuinely are one is scoring an *already-known* sequence — teacher forcing — and there this codebase does exactly that, with the same kernels training uses.) Whether that 2:1 is worth taking is the next question.

And that 2:1 proposal deserves a careful answer: during the rollout, store every token's activations (not just the KV cache), then — once the sequence finishes and the environment returns a reward — backprop through what you stored. Structurally the *waiting* part is exactly right: RL does hold the whole completion and its reward before computing the policy gradient. But in practice you don't backprop through activations saved *during decode*, for two reasons. First, memory — and it's decisive: keeping every layer's activations for a long generated sequence is enormous, precisely the cost activation checkpointing exists to avoid (see §7) — on 16 GB this codebase recomputes forwards even *inside training* rather than store them, and storing across an entire rollout is strictly worse. It's also a bad trade even when it fits: what reuse skips is the *cheapest* phase of the step, and the saving shrinks at every level you look. FLOP accounting says the forward is a third of training — but only the *layer-stack* forward is skippable (the vocab-head forward must re-run regardless: backprop's `dlogits` needs the full-vocab softmax at every position, and storing *that* is ~1.2 GB per 2K tokens), the backward is slower per FLOP than the forward (attention backward is 2.5× its forward; wgrad GEMM shapes are less efficient), and the optimizer adds zero-FLOP wall time — so the skippable slice is ~10% of a training step's wall clock. And since rollout dominates an RL step (~85% here), the end-to-end saving lands near **~1.5%**: a gigabytes-for-1.5% trade, against a backward — where the time actually lives — that no amount of storing can skip. Second, in the general two-system world a naive decode forward is numerically a *different* computation from prefill, so the activations saved during rollout would not equal what a training prefill computes for the same tokens — backprop through them would be biased. The standard answer is therefore **recompute**: when the sequence is complete, run one high-MFU prefill forward over it and backprop through *that*. ANCORA's twist is that its decode is made bitwise-identical to prefill, so the most valuable part of the rollout — the logprobs — *is* reused, verbatim and bitwise (`ratio = 1`, no importance sampling); and since decode equals prefill bit for bit, stored rollout activations *would* even be bit-identical to the prefill's. The backward rides on a recomputed prefill anyway purely because of the trade above: a cheap compute-bound forward beats a rollout-length activation store.
