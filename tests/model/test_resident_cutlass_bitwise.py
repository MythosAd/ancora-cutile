"""CUTLASS MXFP8 GEMM in the BITWISE (ratio=1) path — rollout decode vs training prefill.

Same proof as test_resident_decode.py but the 7 projection GEMMs run on the CUTLASS sm_120a
blockscaled kernel (cutlass=True) in BOTH the training-prefill (ResidentLayerTrain) and the
rollout-decode (ResidentDecodeLayer) stacks. Because the CUTLASS GEMM is M-invariant / no split-K
(tests/hardware/test_cutlass_minvariant.py) and both paths derive the same column-major fp8 weight
from the same bf16 master with the same _quant_mxfp8_w_cm, the frontier row stays bitwise-equal:

  (A) decode hidden[t]  ==  prefill hidden[t]   bitwise, every position
  (B) rollout logprob   ==  training prefill logprob  bitwise  ⇒ π_train/π_infer = 1 exactly
  (C) decode deterministic across runs

So the L2-bound cuda-tile MXFP8 GEMM can be replaced by the faster CUTLASS GEMM without breaking
on-policy correctness. Run:  python tests/model/test_resident_cutlass_bitwise.py [n_layers] [S]
"""
import sys, os, time
...
def _free_obj(objs):
    ...
    def visit(o):
        if isinstance(o, _DBuf):
            if int(o.ptr) not in seen: seen.add(int(o.ptr)); o.free()
...
def prefill(layers_w, x0_all, S):
    layers = [ResidentLayerTrain(cfg, w, 1, S, sr_grad=False, cutlass=True) for w in layers_w]
    ...
    out = gx.to_numpy().copy(); gin0.free(); _free_obj(layers)
    return out


def decode_rollout(layers_w, x0_all, S, maxS, runs=2):
    Md = MGEMM
    layers = [ResidentDecodeLayer(cfg, w, 1, maxS, cutlass=True) for w in layers_w]
...
def boundary_logprob(hidden_pre, embed, final_norm, labels, eps, S):
    V = embed.shape[0]
    h, _ = rmsnorm_forward(hidden_pre, final_norm, si, eps)
...
def main(NL=2, S=128):
    maxS, V = S, 2048
    print(f"CUTLASS MXFP8 in the bitwise path — decode vs prefill   NL={NL} S={S} V={V}")
    print("=" * 76)
    ...
    eb = _f32bf(embed)
    x0_all = (eb[ids].astype(np.uint32) << 16).view(np.float32).astype(np.float32)
    ...
    print("-" * 76)
    same_rows = (bitsf(pre) == bitsf(dec)).all(axis=1); nbad = int((~same_rows).sum())
    print(f"  (A) hidden bitwise: {S - nbad}/{S} positions identical, max|Δ|={np.abs(pre-dec).max():.3g}  "
          f"{'OK' if nbad == 0 else 'FAIL'}")
    if nbad:
        for t in np.where(~same_rows)[0][:6]:
    ...
    print("=" * 76)
    print(f"  {'PASS — CUTLASS GEMM keeps rollout BITWISE-equal to training (ratio=1)' if ok else 'FAIL'}")
    return ok
