...
def newton_schulz_resident_e(gX, gA, gA2, gB, gBX, grecip, E, M, N, si, steps=5, eps=1e-7, schedule=_KELLER):
    """Batched resident NS over E SQUARE experts packed (E*M, N), N==M. gX (E*M,N) bf16 bits
    (the momentum-Nesterov, OVERWRITTEN with the orthogonalized result); gA/gA2/gB/gBX (E*M,M) bf16
    scratch; grecip (E,1) f32. One chain, all experts, no host round-trip.
    schedule defaults to the process-wide setting (Keller unless set_polar_express(True)); pass
